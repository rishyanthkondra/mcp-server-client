...
from sync_base_session import BaseSession, RequestResponder
...
class LoggingFnT(Protocol):
    def __call__(self, params: types.LoggingMessageNotificationParams) -> None: ...
...
class ClientSession(
    ...
    def __init__(
        self,
        read_queue: queue.Queue[types.JSONRPCMessage | Exception],
        write_queue: queue.Queue[types.JSONRPCMessage],
    ...
    def initialize(self) -> types.InitializeResult:
        """Initialize the session by performing a handshake."""
        try:
            ...
            # Make sure to close resources to prevent hangs
            self.close()
            raise
